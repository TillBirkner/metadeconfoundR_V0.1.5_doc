  ...
  r = foreach::foreach(i = seq_along(features), .combine='rbind') %dopar% {
    ...
    for (j in seq_along(covariates)) {
      ...
      if (!is.na(randomVar)) {
        ...
        # if(verbosity == "debug"){
        #   write(paste("I have come this far_129", i,  sep = "\t"),
        #         file = "LRT_pValue.txt",
        #         append = TRUE)
        # }
        ...
        mixedmodel1 <- eval(
          parse(
            text = as.character(
              paste0 (
                "lme4::lmer (rank (FeatureValue) ~ ",
                aCovariate,
                randomVar,
                ", data = subMerge, REML = FALSE)"))))

        # if(verbosity == "debug"){
        #   write(paste("I have come this far_150", i,  sep = "\t"),
        #         file = "LRT_pValue.txt",
        #         append = TRUE)
        # }
        ...
        mixedmodel2 <- eval(
          parse(
            text = as.character(
              paste0 (
                "lme4::lmer (rank (FeatureValue) ~ ",
                substr(randomVar, 2, nchar(randomVar)),", data = subMerge, REML = FALSE)"))))
      ...
      confounders <- NULL
        if (length (lCovariates) > 0 &&
          ...
          for (anotherCovariate in otherCovariates) {
            ...
            if (!is.na(randomVar)) { # switch to mler and REML = FALSE when randomEffects are included
