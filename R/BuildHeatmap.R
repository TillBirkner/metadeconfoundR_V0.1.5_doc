...
BuildHeatmap <- function(metaDeconfOutput,
  ...
  effectSize <- melt(data = metaDeconfOutput$Ds,
  ...
  fdr <- melt(data = metaDeconfOutput$Qs,
              varnames = c(taxon, "metaVariable"),
              value.name = "Qs")

  status <- melt(data = metaDeconfOutput$status,
                 varnames = c(taxon, "metaVariable"),
                 value.name = "status")
  ...
  if (coloring == 1) {
    effectSize$EffectSize[insignificant] <- 0.000001
  }
  if (coloring == 2) {
    effectSize$EffectSize[!trueDeconf] <- 0.000001
  }
  ...
  eff_cast <- dcast(effectSize, effectSize[[1]]~metaVariable, value.var = "EffectSize")
